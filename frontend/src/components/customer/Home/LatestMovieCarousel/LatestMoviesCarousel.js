...
function LatestMoviesCarousel() {
  ...
  return (
    <div className="latest-movie-carousel-page">
      <div className="carousel-section">
        <div className="container">
          ...
          <div className="carousel-container">
            ...
            <div className="latest-movie-grid">
              {visibleMovies.map((movie) => (
                <div key={movie.id} className="latest-movie-card">
                  <div className="latest-movie-poster-container">
                    <img 
                    ...
                    {movie.rating?.average && (
                      <div className="latest-movie-rating-badge">
                  ...
                  <div className="latest-movie-info">
                    ...
                    <div className="latest-movie-meta">
                      <div className="latest-movie-meta-item">
                        <Clock className="h-3 w-3" />
                        <span>{movie.duration ? formatDuration(movie.duration) : 'N/A'}</span>
                      </div>
                      {movie.releaseDate && (
                        <>
                          <span className="latest-movie-meta-separator">•</span>
                          <span>{new Date(movie.releaseDate).getFullYear()}</span>
                        </>
                      )}
                    </div>
                    <div className="latest-movie-genres">
                      {movie.genres?.slice(0, 2).map((genre, idx) => (
                        <span key={idx} className="latest-movie-genre-tag">
                          {genre}
                        </span>
                      ))}
                    </div>
