...
const ScreenSetup = () => {
  ...
  useEffect(() => {
    if (theaterId) {
      dispatch(fetchTheaterById(theaterId)); //updates currentTheater and invokes next useEffect
      console.log("in fetchTheaterById called useEffect");
  ...
  useEffect(() => {
    if (currentTheater && screenId) {
      ...
      if (existingScreen) {
        console.log("Screen Layout:", existingScreen.layout);
        initializeExistingScreen(existingScreen); //filling the fields
      }
    }
  ...
  const initializeExistingScreen = (screen) => {
    console.log("in initializeExistingScreen called");
  ...
  const setupGridFromLayout = (layout) => {
    console.log("in setupGridFromLayout called");
    
    if (!layout?.totalRows || !layout?.totalColumns) {
      console.warn('Invalid layout dimensions:', layout);
      return;
    }
    ...
    layout.stairs?.forEach((stair) => {
      if (stair.location) {
        const row = stair.location.row - 1;
        const col = stair.location.column - 1;
        if (row >= 0 && row < layout.totalRows && col >= 0 && col < layout.totalColumns) {
    ...
    layout.exits?.forEach((exit) => {
      if (exit.location) {
        const row = exit.location.row - 1;
        const col = exit.location.column - 1;
        if (row >= 0 && row < layout.totalRows && col >= 0 && col < layout.totalColumns) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const screenData = {
        ...
        layout: layoutData,
        features: {
          // Changed from screenFeatures to features to match DTO
          screenWidth: screenConfig.screenFeatures.screenWidth,
          screenHeight: screenConfig.screenFeatures.screenHeight,
          projectorType: screenConfig.screenFeatures.projectorType,
          soundSystem: screenConfig.screenFeatures.soundSystem,
        },
        totalSeats,
      };
  ...
  const getCellStyle = (cell) => {
    ...
    if (cell.type === "SEAT") {
      ...
      return {
        ...baseStyle,
        ...gridStyles[category?.color || "empty"]
      };
    }
                    ...
                    <Form.Check
                      ...
                      onChange={() => {
                        setScreenConfig((prev) => ({
                          ...prev,
                          supportedExperiences:
                            prev.supportedExperiences.includes(exp.value)
                              ? prev.supportedExperiences.filter(
                                  (e) => e !== exp.value
                                )
          ...
          <Card>
            <Card.Header className="bg-primary text-white d-flex justify-content-between align-items-center">
              ...
              <div className="d-flex gap-2">
                ...
                <Button variant="light" size="sm" onClick={handleClearGrid}>
            ...
            <Card.Body className="p-0">
              ...
              <div className="p-3 border-top">
                ...
                <div className="d-flex flex-wrap gap-3">
                  {categories.map((category) => (
                    ...
                    >
                      <div
                        ...
                        style={{
                          backgroundColor:
                            gridStyles[category.color]?.backgroundColor,
                        }}
                      ></div>
                      <span>
                        {category.label} - ₹{category.basePrice}
                  ...
                  <div className="d-flex align-items-center gap-2">
                    <div className="legend-box" style={gridStyles.stairs}>
                      ↑
                    </div>
                    <span>Stairs</span>
                  </div>
                  <div className="d-flex align-items-center gap-2">
                    <div className="legend-box" style={gridStyles.exit}>
                      EXIT
                    </div>
                    <span>Exit</span>
                  </div>
